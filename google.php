...
// Include file gpconfig
    ...
    if(empty($user)){ // Jika User dengan email tersebut belum ada
        ...
        $sql = $pdo->prepare("INSERT INTO user(username, nama_lengkap) VALUES(:username,:nama_lengkap)");
        $sql->bindParam(':username', $username);
        $sql->bindParam(':nama_lengkap', $nama);
        // $sql->bindParam(':email', $email);
        $sql->execute(); // Eksekusi query insert
        $user_id = $pdo->lastInsertId(); // Ambil id user yang baru saja di insert
    }else{
        $user_id = $user['user_id']; // Ambil id pada tabel user
